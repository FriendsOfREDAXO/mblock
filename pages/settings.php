...
$config = rex_post('config', array(
    ...
    array('mblock_replace_language_placeholders', 'boolean'),
...
if ($config['submit']) {
    ...
    $this->setConfig('mblock_replace_language_placeholders', $config['mblock_replace_language_placeholders']);
...
// Language Placeholders configuration
$formElements = array();
$elements = array();
$elements['label'] = '
  <label for="rex-mblock-config-replace-language-placeholders">' . rex_i18n::msg('mblock_replace_language_placeholders') . '</label>
';
// create select
$select = new rex_select;
$select->setId('rex-mblock-config-replace-language-placeholders');
$select->setSize(1);
$select->setAttribute('class', 'form-control');
$select->setName('config[mblock_replace_language_placeholders]');
// add options
$select->addOption(rex_i18n::msg('mblock_disabled'), 0);
$select->addOption(rex_i18n::msg('mblock_enabled'), 1);
$select->setSelected($this->getConfig('mblock_replace_language_placeholders', 0)); // Default: disabled
$elements['field'] = $select->get();
$formElements[] = $elements;
// parse select element
$fragment = new rex_fragment();
