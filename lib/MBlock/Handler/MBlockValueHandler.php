...
class MBlockValueHandler
{
    ...
    public static function loadRexVars()
    {
        ...
        // GRIDBLOCK INTEGRATION: Check for fake SQL result first
        // Gridblock setzt sql_fake_result wenn es ein Modul innerhalb von Gridblock lädt
        $fakeResult = rex::getProperty('sql_fake_result');
        if ($fakeResult !== null && is_array($fakeResult)) {
            // Gridblock hat fake Daten gesetzt - diese verwenden
            for ($i = 1; $i <= 20; $i++) {
                if (isset($fakeResult['value' . $i])) {
                    $result['value'][$i] = $fakeResult['value' . $i];
                    
                    // JSON-Dekodierung wenn nötig
